	...
	<div class="container">
		<div class="row">
			<div class="col-8 mx-auto text-center">
				<h2 class="mb-3 text-capitalize">Featured Destination</h2>
				<ul class="list-inline breadcrumbs text-capitalize" style="font-weight:500">
					<li class="list-inline-item"><a href="{{ route('home') }}">Home</a>
					</li>
					<li class="list-inline-item">/ &nbsp; <a href="{{ route('destination') }}">Destination</a>
					</li>
				</ul>
			</div>
		</div>
...
<section class="section"> 
    <div class="container">
        <div class="widget widget-categories text-center">
            <h5 class="widget-title mb-4"><span>Category</span></h5>
            @if ($categories->isNotEmpty())
                <ul class="list-unstyled widget-list d-flex flex-wrap justify-content-center gap-3">
                    {{-- See All Button --}}
                    <li>
                        <a wire:navigate 
                    ...
                    @foreach ($categories as $category)
                        <li>
                            <a wire:navigate 
                               href="{{ route('destination') . '?categorySlug=' . $category->slug }}" 
                            ...
                            @foreach ($destinations as $destination)
                                <div class="col-md-4" data-aos="fade">
                                    <article class="blog-post h-100">
                                        ...
                                        <div class="pt-4">
                                            <h2 class="h4">
                                                <a class="text-black" href="{{ route('destinationDetail', $destination->id) }} ">
                                                    {{ $destination->title }}
                                                </a>
                                            </h2>
                                            <p>{{ $destination->short_desc }}</p>
                                            <a wire:navigate href="{{ route('destinationDetail', $destination->id) }}" class="text-info fw-bold">Read More</a>
                                        </div>
                                    </article>
                                </div>
                            @endforeach
